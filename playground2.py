...
def test_moe():
    ...
    # NOTES
    # expert_output_size is the output units per expert
    # output_size is the total output units/labels

    # net = moe.MoE(input_size=768, expert_output_size=2, output_size=10, num_experts=10, hidden_size=256, noisy_gating=True, k=2, kind=kind)
    net = moe.MoE(input_size=768, expert_output_size=100, output_size=100, num_experts=50, hidden_size=256, noisy_gating=True, k=20, kind=kind)
    net = net.to(device)
