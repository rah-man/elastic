        ...
        for task in range(len(self.dataset)):  
            ...
            for epoch in range(self.epochs):
                ...
                for i, (x, y) in enumerate(trainloader):
                    ...
                    if self.previous_model and isinstance(self.regulariser, LwF):
                        self.previous_model.eval()
                        with torch.no_grad():
                            prev_dist, _, _, _ = self.previous_model(x, y)
                        exit()
                        distillation_loss = self.regulariser.calculate_loss(outputs, prev_dist, self.seen_cls)
