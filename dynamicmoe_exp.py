...
class DynamycMoE(nn.Module):
    ...
    def expand_expert(self, seen_cls, new_cls, k=2):
        ...
        if not self.experts:
            ...
            # 20221121 - EXPERIMENTAL TWO TASKS AT INITIATION
            self.seen_cls = 2 * new_cls
            experts = nn.ModuleList([MLPDynamic(input_size=self.input_size, hidden_size=self.hidden_size, output_size=new_cls, projected_output_size=new_cls) for _ in range(2)])
            self.num_experts = len(experts)

            for expert_index, module in enumerate(experts):
                module.mapper = nn.Linear(in_features=self.input_size, out_features=(self.seen_cls), bias=False)
