...
class ExemplarsDataset(MemoryDataset):
    ...
    def __init__(self, transform, class_indices,
                 num_exemplars=0, num_exemplars_per_class=0, exemplar_selection='random', is_feature=0):
        super().__init__({'x': [], 'y': []}, transform, class_indices=class_indices, is_feature=is_feature)
    ...
    def extra_parser(args):
        ...
        # MODIFIED BELOW
        parser.add_argument('--is-feature', default=0, type=int, required=False,
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform, is_feature=False):
        if self._is_active():
